...
```bash
mammamia-tags$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mammamia-tags$ sam local start-api
mammamia-tags$ curl http://localhost:3000/
```
...
```bash
mammamia-tags$ sam logs -n HelloWorldFunction --stack-name mammamia-tags --tail
```
